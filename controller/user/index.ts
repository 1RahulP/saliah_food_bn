...
export const handleUserProfileController = async (req: Request, res: Response) => {
    try {
        upload(req, res, async (err) => {
            ...
            const profile = uploadedFiles.map(file => `${process.env.BASE_URL}/profile/${file.filename}`);
            try {
                await Promise.all(profile.map(async (filePath) => {
...
export const paymentVerification = async (req: Request, res: Response) => {
    ...
    console.log({ razorpay_order_id }, { razorpay_payment_id }, { razorpay_signature }, { event });
    ...
    if (!razorpay_signature) {
        ...
        return res.status(400).json({ success: false, message: "razorpay_signature is missing" });
    }
